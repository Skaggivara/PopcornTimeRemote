...
- (void)loadView
{
    ...
    [self.desc setText:@"Connect this device to the same network as your computer running Popcorn Time."];
    ...
    [self.scanBtn setTitle:@"Find Popcorn Time" forState:UIControlStateNormal];
    ...
    [self.connectBtn setTitle:@"Connect to IP address*" forState:UIControlStateNormal];
    ...
    [self.footnote setText:@"*Your computers IP address"];
    ...
    [self.notice setText:@"This remote control works with Popcorn Time version 3.3 and above found at www.getpopcorntime.io"];
...
- (void)handleInput
{
    if(![[AFNetworkReachabilityManager sharedManager] isReachableViaWiFi]){
        //update text
        [[[UIAlertView alloc] initWithTitle:@"Problem Connecting" message:[NSString stringWithFormat:@"Please connect to the same network (WIFI) as your computer running popcorn time."] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil] show];
    } else {
        
        POPDevice *existing = [POPDevice savedAddress];
        
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Connect" message:@"Enter the IP address of the computer running Popcorn Time:" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Connect", nil];
        alertView.alertViewStyle = UIAlertViewStylePlainTextInput;
        
        if(existing){
            [alertView textFieldAtIndex:0].text = existing.address;
        }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if([title isEqualToString:@"Connect"])
    {
        ...
        if(![self.scanner isIpAddressValid:address] || a.count < 4){
            [[[UIAlertView alloc] initWithTitle:@"Invalid address" message:[NSString stringWithFormat:@"Please use a valid IP address."] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil] show];
        } else {
            ...
            [POPNetworking validatePopcorn:address
                                   ...
                                   success:^(id responseObject)
             {
                 ...
                 [[[UIAlertView alloc] initWithTitle:@"Popcorn Time not found" message:[NSString stringWithFormat:@"Please make sure Popcorn Time is runnning."] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil] show];
             }
             ];
...
- (void)handleScan
{
    if(![[AFNetworkReachabilityManager sharedManager] isReachableViaWiFi]){
        ...
        if (!self.hud) {
            ...
            self.hud.labelText = @"Searching... (Tap to cancel)";
...
- (void)scanLANDidFinishScanning {
    ...
    // timeout then alert and hide hud
    
    [[[UIAlertView alloc] initWithTitle:@"Scan Finished" message:[NSString stringWithFormat:@"Number of devices connected to the Local Area Network : %i", (int)self.connectedDevices.count] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil] show];
...
- (void)checkExisting
{
    ...
    if(existing && [[AFNetworkReachabilityManager sharedManager] isReachableViaWiFi]){
        ...
        hud.mode = MBProgressHUDModeIndeterminate;
        hud.backgroundColor = [UIColor colorWithWhite:0.0 alpha:.5];
        hud.labelText = @"Connecting to existing...";
        ...
        [POPNetworking validatePopcorn:[existing validAddress] port:kPopcornPort user:kPopcornUser password:kPopcornPass success:^(id responseObject) {
            ...
            [[[UIAlertView alloc] initWithTitle:@"Problem Connecting" message:[NSString stringWithFormat:@"There was a problem connecting to your saved computer, please try the scan again."] delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil] show];
